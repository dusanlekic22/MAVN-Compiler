...
ostream& operator<<(ostream& out, Instruction& i)
{
	...
	case I_SW: // sw rid,num(rid)
		out << "sw $t" << i.m_src.front()->getAssignment();
		v = i.m_src.back();
		out << ", " << (reinterpret_cast<ConstInstruction*>(&i))->getNumValue()
			<< "($t" << v->getAssignment() << ")";
		break;
	...
	case I_DIV: //div rid,rid,rid
		out << "div $t" << i.m_dst.front()->getAssignment();
		for (Variables::iterator it = i.m_src.begin(); it != i.m_src.end(); it++)
			out << ", $t" << (*it)->getAssignment();
		break;
	
	case I_XOR: //xor rid,rid,rid
		out << "xor $t" << i.m_dst.front()->getAssignment();
		for (Variables::iterator it = i.m_src.begin(); it != i.m_src.end(); it++)
			out << ", $t" << (*it)->getAssignment();
		break;

	case I_SB: // sb rid,num(rid)
		out << "sb $t" << i.m_src.front()->getAssignment();
		v = i.m_src.back();
		out << ", " << (reinterpret_cast<ConstInstruction*>(&i))->getNumValue()
			<< "($t" << v->getAssignment() << ")";
		break;
